...
const DetailItem = ({ icon, label, value }: { icon: React.ReactNode, label: string, value: string | number }) => (
    <div className="flex items-start p-4 bg-white/5 rounded-xl">
        <div className="text-teal-300 mr-4 mt-1">{icon}</div>
        <div>
    ...
    return (
        <div className="animate-fade-in p-6 h-full flex flex-col">
            <header className="relative flex items-center justify-center mb-6">
                <button onClick={onBack} className="absolute left-0 text-teal-300 hover:text-white transition-colors" aria-label="Go back">
                    <ArrowLeftIcon className="w-6 h-6" />
                </button>
                <h2 className="text-2xl font-semibold text-teal-200 truncate px-10" title={trip.activity}>{trip.activity}</h2>
            </header>
            
            <div className="flex-grow overflow-y-auto scrollbar-hide space-y-4">
                <div className="text-center mb-4 p-4 bg-black/10 rounded-xl">
                    <p className="font-bold text-2xl leading-tight truncate" title={trip.origin}>{trip.origin}</p>
                    <p className="text-gray-400 text-lg my-1">to</p>
                ...
                {hasPathData ? (
                    ...
                    <div className="h-56 rounded-xl bg-black/20 flex items-center justify-center text-center text-gray-400 p-4">
