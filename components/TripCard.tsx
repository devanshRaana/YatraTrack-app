...
const transportIcons: Record<TransportMode, React.ReactNode> = {
    [TransportMode.CAR]: <CarIcon className="w-5 h-5 text-teal-300" />,
    [TransportMode.BUS]: <BusIcon className="w-5 h-5 text-teal-300" />,
    [TransportMode.TRAIN]: <TrainIcon className="w-5 h-5 text-teal-300" />,
    [TransportMode.BICYCLE]: <BicycleIcon className="w-5 h-5 text-teal-300" />,
    [TransportMode.WALK]: <WalkIcon className="w-5 h-5 text-teal-300" />,
    [TransportMode.AUTO]: <CarIcon className="w-5 h-5 text-teal-300" />, // Using car as a proxy
    [TransportMode.OTHER]: <OtherIcon className="w-5 h-5 text-teal-300" />,
};
...
const TripCard: React.FC<TripCardProps> = ({ trip, onEdit, onDelete, onClick }) => {
  ...
  return (
    <div 
      className="bg-white/10 p-4 rounded-2xl shadow-lg backdrop-blur-lg transition-all hover:scale-105 hover:bg-white/20 duration-300 animate-fade-in flex flex-col cursor-pointer"
    ...
    >
      ...
      <div className="flex-grow">
        <div className="mb-3">
            ...
            <p className="text-gray-400 text-sm my-0.5">to</p>
            <p className="font-bold text-lg leading-tight truncate" title={trip.destination}>{trip.destination}</p>
        </div>
        
        <div className="text-sm text-teal-200 mb-4">{trip.activity}</div>
        
        <div className="flex items-center justify-between text-sm text-gray-300">
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 bg-teal-900/50 py-1 px-2.5 rounded-full">
                    {transportIcons[trip.mode]}
                    <span className="text-xs font-medium">{trip.mode}</span>
                </div>
      ...
      <div className="flex justify-between items-center text-xs text-gray-400">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex items-center gap-1.5 py-1 px-2 rounded-md text-teal-300 hover:bg-white/10 hover:text-white transition-colors disabled:text-gray-500 disabled:hover:bg-transparent"
            ...
            <button
              ...
              className="flex items-center gap-1.5 py-1 px-2 rounded-md text-teal-300 hover:bg-white/10 hover:text-white transition-colors"
            ...
            <button
              ...
              className="flex items-center gap-1.5 py-1 px-2 rounded-md text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-colors"
      ...
      {isMapVisible && (
        <div className="mt-3" onClick={(e) => e.stopPropagation()}>
          {hasPathData ? (
            ...
            <div className="p-4 bg-black/20 rounded-lg text-center text-sm text-gray-400">
